...
def CNTS(digital_title, digital_sub_title, digital_date, subtitle):
    ...
    try:
        ...
        f_error.write(digital_title+"/"+digital_sub_title +
                      "/"+digital_date+"\n")
        return 2
...
total_num = total_num.replace(",", "")
url = f'https://www.nl.go.kr/NL/contents/N20302010000.do?searchType=month&schM=day_list&page=1&pageSize={total_num}&searchYear={str(year)}&searchMonth='+str(
    month)
req = requests.get(url)
html = req.text
soup_digital = BeautifulSoup(html, 'html.parser')
...
while i < int(total_num):
    print('number_'+str(i)+'    .........'+str(int(total_num)-i)+' left...')
    ...
    if control_num_ == 1:
        ...
        if subtitle:
            data = digital_title+" : "+digital_sub_title + "/" + digital_date + "/" +\
